                ...
                <div class="box-body">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2" name="category_parent" style="width: 100%;">
                            ...
                            @foreach($list as $cat)
                            <option value="{{$cat->id}}" {{$cat->id == $detail->parentId ? 'selected' : ''}} >{{$cat->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    {{CommonHelperFn\ajaxUploadFile(['label'=>'Ảnh danh mục','name'=>'image','id'=>'category_image','value'=>RewriteUrlFn\get_picture_path($detail->image)])}}
                    <div class="form-group">
                        <label>Biểu tượng</label>
                        <div class="row">
                            @foreach($listIcon as $key=>$item)
                                <div class="col-xs-2">
                                    <label>
                                        <input type="radio" class="form-control iCheck"
                                               {{$detail->icon == $key ? "checked" : ""}}
                                               name="category_icon" value="{{$key}}">
                                        <i class="{{$item}}"></i>
                                    </label>
                                </div>
                            @endforeach
